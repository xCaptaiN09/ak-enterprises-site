const rows = [
  ...
  { label: 'Hours', value: 'Mon-Sat 9:00-20:00 / Sun 10:00-20:00' },
];

export default function Footer() {
  return (
    <footer id="contact" className="bg-ink px-6 pt-20 pb-10 md:px-10">
      <div className="mx-auto w-full max-w-[1600px]">
              ...
              <div key={r.label} className="grid grid-cols-[110px_1fr] items-baseline gap-4 border-b border-white/10 py-5 first:border-t">
                ...
                {r.href ? (
                  <a
                    ...
                    rel="noreferrer"
                    className="group text-lg text-white transition-colors hover:text-neon md:text-xl"
                  >
                    {r.value}
                    {r.href.startsWith('http') && (
                      <span className="ml-2 font-mono text-[10px] text-white/30 transition-colors group-hover:text-neon">↗</span>
                    )}
                  </a>
                ) : (
                  <span className="text-lg text-white md:text-xl">{r.value}</span>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mt-20 flex items-end justify-between gap-10">
          <h2 className="display leading-none text-[16vw] text-white/85 md:text-[11rem]">/ Contact</h2>
          <img src={`${import.meta.env.BASE_URL}img/logo-white.svg`} alt="AK Enterprises" className="h-32 w-auto md:h-44" />
        </div>
        ...
        <div className="mt-16 flex flex-col gap-3 font-mono text-[10px] tracking-widest text-white/35 uppercase md:flex-row md:items-center md:justify-between">
          ...
          <a href="#home" className="transition-colors hover:text-neon">Back to top ↑</a>
        </div>
      </div>
    </footer>
  );
